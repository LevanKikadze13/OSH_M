...
function Register() {
  ...
  const validatePhoneNumber = (phoneNumber) => {
    // Regular expression pattern for Georgian phone numbers
    const georgianPhonePattern = /^(\+995|0)?\d{9}$/;
    return georgianPhonePattern.test(phoneNumber);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (!validatePhoneNumber(formData.phoneNumber)) {
      setPhoneNumberError('შეიყვანეთ არსებული ტელეფონის ნომერი!');
      return;
    }

    // Reset phone number error if validation passes
    setPhoneNumberError('');
                ...
                <form onSubmit={handleSubmit}>
                  ...
                  <div className="form-group mb-3">
                    ...
                    <input
                      ...
                      onChange={handleChange}
                      pattern="^[0-9+]+$"
                      title="Please enter a valid Georgian phone number (digits and + only)"
                    />
